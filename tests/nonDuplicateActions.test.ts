...
describe("toggleNonDuplicates", () => {
  ...
  // -------------------------------------------------------------------------
  // Dual-write to SyncedSettings
  // -------------------------------------------------------------------------

  test("mark writes pairs to SyncedSettings via addPair", async () => {
    const mockSyncStore = {
    ...
    getItemMock.mockImplementation((input: any) => {
      if (Array.isArray(input)) {
        return input.map((id: number) => ({ id, libraryID: 77, key: `KEY${id}` }));
      }
      return { id: input, libraryID: 77, key: `KEY${input}` };
    });

    await toggleNonDuplicates("mark", [10, 20], 77, { syncStore: mockSyncStore as any });

    expect(mockSyncStore.addPair).toHaveBeenCalledWith(77, "KEY10", "KEY20");
    expect(mockSyncStore.removePair).not.toHaveBeenCalled();
  });

  test("unmark removes pairs from SyncedSettings via removePair", async () => {
    const mockSyncStore = {
      addPair: jest.fn<(...args: any[]) => Promise<void>>(async () => {}),
      removePair: jest.fn<(...args: any[]) => Promise<void>>(async () => {}),
    };
    ...
    await toggleNonDuplicates("unmark", [10, 20], 77, { syncStore: mockSyncStore as any });

    expect(mockSyncStore.removePair).toHaveBeenCalledWith(77, "KEY10", "KEY20");
    expect(mockSyncStore.addPair).not.toHaveBeenCalled();
  });

  test("SyncedSettings write failure does not block local DB write", async () => {
    const mockSyncStore = {
      addPair: jest.fn<(...args: any[]) => Promise<void>>().mockRejectedValue(new Error("sync fail")),
      removePair: jest.fn<(...args: any[]) => Promise<void>>(async () => {}),
    };
    getItemMock.mockImplementation((input: any) => {
      if (Array.isArray(input)) {
    ...
    // Local DB write still happened
    expect(insertNonDuplicatesMock).toHaveBeenCalledWith([10, 20], 77);
    // Duplicate refresh still happened
    expect(fetchDuplicatesMock).toHaveBeenCalled();
  });

  test("skips SyncedSettings write when item keys are undefined", async () => {
    const mockSyncStore = {
      addPair: jest.fn<(...args: any[]) => Promise<void>>(async () => {}),
      removePair: jest.fn<(...args: any[]) => Promise<void>>(async () => {}),
    };
    getItemMock.mockImplementation((input: any) => {
      ...
      return { id: input, libraryID: 77 }; // no key property
    });
    ...
    expect(mockSyncStore.addPair).not.toHaveBeenCalled();
    // Local DB write still happened
    expect(insertNonDuplicatesMock).toHaveBeenCalled();
  });

  test("skips SyncedSettings write when item keys are empty string", async () => {
    const mockSyncStore = {
      addPair: jest.fn<(...args: any[]) => Promise<void>>(async () => {}),
      removePair: jest.fn<(...args: any[]) => Promise<void>>(async () => {}),
    };
    getItemMock.mockImplementation((input: any) => {
      if (Array.isArray(input)) {
        return input.map((id: number) => ({ id, libraryID: 77, key: "" }));
      }
      return { id: input, libraryID: 77, key: "" };
    });

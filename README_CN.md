...
这个版本主要完成 Zotero 9–10 兼容和数据一致性改进：

1. ✨ **新增**：Zoplicate 现在面向 **Zotero 9 和 10**（`9.0` 到 `10.0.*`），发布清单也包含 Zotero 10 的兼容信息。
2. ✨ **新增**：非重复条目关系现在可以通过 `Zotero.SyncedSettings` 同步，并使用条目 key 存储，便于跨设备迁移。
3. 🧬 **变更**：重新审查了重复检测、弹窗渲染、菜单注册、窗口处理和插件生命周期代码，以适配 Zotero 9 和 10 运行环境。
...
1. Zoplicate 5 面向 **Zotero 9 和 10**（`9.0` 到 `10.0.*`）。
...
4. 你可以在**数据管理**部分**导出**和**导入**非重复条目记录。
   * 该功能可用于在不同设备间迁移非重复条目数据。详见[导入/导出非重复条目](#导入导出非重复条目)。
...
> [!TIP]
>
> 如果有大量重复条目需要合并，你可以在设置（编辑 > 设置 > Zoplicate）中调大**并行合并**数量
> （1–8，默认为 3），以同时合并多个重复组来加快处理速度。设为 `1` 可恢复为之前的串行行为。
...
非重复条目记录保存在本地数据库中，不会通过 Zotero 同步服务自动同步到其他设备。
如需将非重复条目记录迁移到其他设备，可以使用导入/导出功能：

1. 前往 `编辑` -> `设置` -> `Zoplicate` -> **数据管理** 部分。
2. 单击 <kbd>导出非重复记录…</kbd>，将所有非重复条目记录保存为 JSON 文件。
3. 将导出的文件复制到另一台设备。
4. 在另一台设备上，前往同一设置面板，单击 <kbd>导入非重复记录…</kbd> 加载该文件。

> [!NOTE]
>
> * 导出文件使用稳定的条目 key（而非本地 ID），因此可以在共享同一同步库的不同 Zotero 设备之间正确工作。
> * 导入操作是**只增不删**的——只会添加新记录，不会删除已有记录。你可以放心多次导入同一文件。
> * 如果导出文件中引用的某个条目在目标设备上不存在（例如尚未同步或已删除），该条记录将被跳过。
